...
## Independent decision layer (Jev)

`decisionProvider` defaults to `development`, including for older persisted records that omit the field. Creative generation remains a separate provider. With Jev selected, the concepts job first persists generated concepts, then runs independent six-question ordinal evaluations through `server/decision-providers.ts`. Generator scores are excluded from the judge's input. Normalized Jev criteria replace the selection scores; the original criteria remain in `generatorCriteria` and creative traces.

`server/decision-policy.ts` owns normalization and probability/confidence gates. It reuses `scoreConcept()` for weighted scoring. Automatic selection requires every concept to have a current independent evaluation and the highest-scoring concept to satisfy the policy. Missing/failed judgments never fall back to generator scores. Manual selection is allowed after a valid evaluation, even if its judgments require review.

A durable `preflight` job evaluates each planned shot using Score and Noul questions. The existing `assets_planned` state remains the generation gate. Assisted mode runs preflight before its human approval pause; autonomous mode proceeds only after the gate passes. Manual mode exposes preflight as a stage. Both generation dispatch and the generation handler enforce the gate, and regeneration invalidates it before new work can be queued. Human overrides require review notes and completed, current evaluations. Their provenance is recorded and invalidates when storyboard input changes. Factual preflight only checks semantic consistency against supplied verified facts. Existing deterministic research/source bindings remain independent.

`server/typesafe-client.ts` uses the official SDK, with a fixed official endpoint, server-only key, bounded body/timeout, job AbortSignal, disabled SDK logging, and zero SDK retries. `server/paid-call.ts` shares budget reservation, settlement and uncertain-outcome recovery across TypeSafe/OpenAI/ElevenLabs. SDK HTTP errors are reduced to safe status/request ID metadata. Unknown outcomes retain reserved cost and block automatic repurchase; bounded 429 retries remain owned by the durable queue.

API-call records own complete request state/questions, raw responses, usage, latency, failure status, and costs. Linked `DecisionEvaluation` records own the typed judgments, normalized criteria, resolved model, rubric version, revision, deterministic reasons, policy snapshot and human review. Cache keys hash the stage, revision, requested model, versioned questions and structured input. Replays reuse completed calls; editing inputs or rubric/model selection yields a new evaluation. Changing thresholds reinterprets existing answers without a new paid request.

Jev sees text only. FFprobe still controls technical export acceptance; Jev judgments never set rendered-media creative review to true. There is no autonomous rewrite loop, publication analytics calibration, or multimodal critic in this increment. See `docs/JEV_SETUP.md` for operational setup and initial thresholds.
